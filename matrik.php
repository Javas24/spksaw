...
<div class="modal-body">
    ...
    <div class="mb-3">
        <label class="form-label font-weight-bold">Nama Alternatif:</label>
    ...
    <hr>
    <h6>Isi Nilai Kriteria:</h6>

    <?php
        // Mengambil semua kriteria dari database
        $q_kriteria = $db->query("SELECT * FROM saw_criterias ORDER BY id_criteria ASC");

        while ($row = $q_kriteria->fetch_object()) {
            echo '<div class="mb-2">';
            echo '<label class="form-label">' . $row->criteria . ' (' . $row->attribute . ')</label>';

            // Perhatikan attribute name="values[id_criteria]"
            // Ini akan mengirim data dalam bentuk ARRAY ke PHP
            echo '<input type="number" step="0.01" name="values[' . $row->id_criteria . ']" class="form-control" placeholder="Nilai untuk ' . $row->criteria . '" required>';
...
<div class="modal-footer">
    ...
    <button type="submit" class="btn btn-primary ml-1">
        ...
        <span class="d-none d-sm-block">Simpan Semua</span>
    </button>
</div>
